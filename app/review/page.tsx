        ...
        if (order.state === 'OS080') {
          for (const product of order.products) {
            ...
            items.push({
              ...
              purchase_date: new Date(order.createdAt).toLocaleDateString('ko-KR', {
                year: 'numeric',
                month: '2-digit',
                day: '2-digit',
              }).replace(/\. /g, '.').replace(/\.$/, ''),
          ...
          <div>
            {availableItems.length === 0 ? (
              ...
              availableItems.map((item) => (
                <div key={`${item.order_id}-${item.product_id}`} className="px-5 py-4">
                  ...
                  <button
                    onClick={() => router.push(`/review/write?order_id=${item.order_id}&product_id=${item.product_id}`)}
                ...
                <div key={review._id} className="px-5 py-4">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <GrayButton
                        text="수정하기"
                        onClick={() => router.push(`/review/edit?reply_id=${review._id}`)}
                      />
                    </div>
                    <div className="flex-1">
                      <GrayButton
